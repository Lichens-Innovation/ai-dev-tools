...
// node_modules under pnpm's strict layout. Find it in the store instead — walk up from this
// script looking for a node_modules/.pnpm directory, since the skill may live at any depth
// inside a project.
function findPnpmStore(startDir) {
  let dir = startDir;
  for (;;) {
    const candidate = join(dir, "node_modules/.pnpm");
    if (existsSync(candidate)) return candidate;
    const parent = dirname(dir);
    if (parent === dir) throw new Error("no node_modules/.pnpm found in any parent directory");
    dir = parent;
  }
}
...
function findSystemDist() {
  ...
  const store = findPnpmStore(scriptDir);
  const dir = readdirSync(store).find((d) => d.startsWith("@xyflow+system@"));
  if (!dir) throw new Error(`@xyflow/system not found in the pnpm store at ${store}`);
...
console.log(`checking against ${dist}\n`);

// ── The model under test ────────────────────────────────────────────────────────────────────
// Swap this for your own implementation to check it.
