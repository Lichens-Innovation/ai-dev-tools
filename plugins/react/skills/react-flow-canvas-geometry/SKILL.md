...
For checking the result visually, drive the app in a real browser/window rather than trusting a
static read of the coordinates.
...
decides geometry ahead of render, such as a layout or seed package computing `label_offset` values
before anything renders. If you are in that position, go to *Checking a reimplementation* below
before you trust it.
...
the library in 51.6% of cases**. In one real graph, a back-route spanning 165px agreed with the
wrong formula to under 1px — while the same model was **34px out** on a 445px span, enough to put a
label inside a node.

**This is why spot-checking two labels in a rendered window proves nothing.** A wrong
reimplementation like this can pass casual review and still ship a real bug.
...
- **Outside the renderer, height is not knowable.** Card *widths* are usually fixed by a class
  ...
  wraps: in one project, five nodes carrying 1–4 chips measured 78/78/103/129/129px against a
  "50 + 30 per chip" model's 80/110/110/170/170, because chips wrap by text width and often share
  a row.
...
Test this against several different height models precisely because the placement's claim is that
...
Note it replaces the cubic with a quadratic, so the curve's *shape* changes as soon as an offset is
non-zero — if you seed offsets, you are also changing how every seeded edge is drawn. Decide
whether you want that; if not, keep the original `edgePath` and move only the label.
...
One trap that belongs to neither: reading a rect after `scrollIntoView` inside the flow pane can
give coordinates that hit-testing does not honour — verify against a real pointer interaction, not
just the DOM rect.
