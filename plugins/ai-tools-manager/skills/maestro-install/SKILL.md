...
description: "Installs the Maestro orchestrator into this project, then opens the visual editor to author the config. Detects the implementation agent(s) from the repo, scaffolds the maestro skill + runtime scripts + settings (bash-validation hook) + gitignore, then hands off to the /maestro-app skill to fill the canvas, write maestro.json, render the orchestrator, and place rules. Use when the user runs /maestro-install, or asks to set up / scaffold / install the Maestro subagents workflow for this project. To re-edit an already-installed project, use /maestro-app instead."
---

# Maestro Install

One-time setup of Maestro for a project: lay down the orchestrator scaffolding, then open the visual editor to author the config. The heavy lifting of the config itself (form → `maestro.json` → render → rules) is delegated to the **`/maestro-app`** skill, so this skill only does the parts `/maestro-app` can't: detecting the project's implementation agent(s) and scaffolding the orchestrator the editor needs.

```
maestro-install:  analyze repo → offer local skills → scaffold orchestrator → run /maestro-app (form + write + render + rules) → report
              (impl agents)   (best-fit + consent)                         └── /maestro-app needs maestro.md to exist; the scaffold step creates it
```
...
4. **Author the config — run the `/maestro-app` skill.** Now that `maestro.md` exists, invoke the **`maestro-app`** skill, passing the detected implementation agents (and, if you assembled one in step 2, the skill map) as its arguments so the canvas opens already seeded for this project:
   ...
   > Run the `maestro-app` skill with arguments: `<implAgents>` (e.g. `backend,frontend`), and the skill map `<json>` (e.g. `{"frontend":["react"],"test":["component-test"]}`) when non-empty.
...
- This skill is the **scaffold + delegate** entry point; the config materialisation (form → `maestro.json` → render → rules) lives entirely in the `/maestro-app` skill so there is one place that owns it.
- Re-running `/maestro-install` is safe: scaffolding is idempotent, and a present `maestro.md` is never overwritten. To just re-edit config on an installed project, prefer `/maestro-app` (it skips the scaffold step).
