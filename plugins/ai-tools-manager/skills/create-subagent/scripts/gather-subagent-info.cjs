...
<div class="card">
  <h1>New Subagent</h1>
  <p class="subtitle">Choose how you want to create the subagent.</p>
    ...
    <div class="form-body">
      <div class="modes">
        <label class="mode-card active" id="cardAuto">
          ...
          <div class="mode-desc">Describe your agent idea in plain language. Claude generates the name, role, full workflow, and AGENTS.md content for you.</div>
        </label>
        <label class="mode-card" id="cardManual">
          <input type="radio" name="_mode" value="manual">
          <div class="mode-title">Manual</div>
          <div class="mode-desc">Provide the name, description, trigger conditions, and tool list yourself. Claude creates a structured AGENTS.md template you fill in.</div>
        </label>
      </div>
      ...
      <div class="fields visible" id="autoFields">
        <div class="field">
          <label>Subagent name</label>
          <input name="name" placeholder="my-agent" pattern="[a-z][a-z0-9-]*"
                 autocomplete="off" spellcheck="false">
          <p class="hint">kebab-case — leave blank to let Claude derive one from your idea</p>
        </div>
        <div class="field">
          <label>Describe your subagent idea</label>
          <textarea name="idea" rows="6"
            placeholder="e.g. A specialized agent that handles all database work — running migrations, writing queries, reviewing schema changes for safety issues, and coordinating with the backend agent for propagation."></textarea>
        </div>
      </div>
      ...
      <div class="fields" id="manualFields">
        <div class="field">
          <label>Subagent name</label>
          <input name="nameManual" placeholder="my-agent" pattern="[a-z][a-z0-9-]*"
                 autocomplete="off" spellcheck="false">
          <p class="hint">kebab-case, e.g. backend-agent</p>
        </div>
        <div class="field">
          <label>Description</label>
          <input name="description" placeholder="Role and responsibilities of this agent">
        </div>
        <div class="field">
          <label>When to apply</label>
          <input name="triggers" placeholder='e.g. "when main thread needs backend tasks done"'>
        </div>
        <div class="field">
          <label>Tools</label>
          <input name="tools" placeholder="e.g. Read, Edit, Bash — comma-separated">
        </div>
      </div>
...
<script>
  ...
  function setMode(mode) {
    ...
    if (mode === 'auto') {
      cardAuto.classList.add('active'); cardManual.classList.remove('active');
      autoFields.classList.add('visible'); manualFields.classList.remove('visible');
      manualFields.querySelectorAll('input,textarea').forEach(el => el.removeAttribute('required'));
      autoFields.querySelector('textarea').setAttribute('required', '');
    } else {
      cardManual.classList.add('active'); cardAuto.classList.remove('active');
      manualFields.classList.add('visible'); autoFields.classList.remove('visible');
      autoFields.querySelectorAll('input,textarea').forEach(el => el.removeAttribute('required'));
      manualFields.querySelectorAll('input').forEach(el => {
        if (el.name !== 'nameManual') el.setAttribute('required', '');
      });
      manualFields.querySelector('[name=nameManual]').setAttribute('required', '');
    }
  }
  ...
  document.getElementById('form').addEventListener('submit', async (e) => {
    ...
    if (payload.mode === 'manual') { payload.name = payload.nameManual; }
    delete payload.nameManual;
    await fetch('/submit', {
  ...
  const output = JSON.stringify({
    ...
    ...(result.mode === 'auto'
      ...
      : {
          ...
          tools: result.tools?.trim(),
        }),
    marketplacePath,
    plugin: result.plugin,
  });

  fs.writeFileSync(path.join(targetDir, 'subagent-gather-info.json'), output, 'utf8');
  console.log(output);
}
