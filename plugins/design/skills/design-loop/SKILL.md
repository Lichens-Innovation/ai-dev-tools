...
| Read component API | Storybook MCP `get-documentation` |
| Which stories changed | Storybook MCP `get-changed-stories` |
| Render for comparison | `scripts/screenshot.mjs` (Playwright) |
| Validate | Storybook MCP `run-story-tests` |
| Publish | `npm run chromatic` |
| Track work | `TaskCreate` / `TaskUpdate` (one task per component) |

## Preconditions

Verify contract §6. Concretely: `design.manifest.json` exists at the repo root; Storybook is
running (`npm run storybook`, default `http://localhost:6006`); the Storybook MCP is reachable;
Playwright is installed; Chromatic is wired. If any is missing, **stop** and point the user at
`design-init` rather than guessing.
...
Storybook MCP `get-documentation` for the component → real props, existing stories, and current
...
2. Find the changed stories: Storybook MCP `get-changed-stories` → the `storyId`s your edit
   touched (cross-check against the manifest's `storyId`).

3. Screenshot each changed story from the running Storybook:
...
Storybook MCP `run-story-tests` for the component → accessibility + interaction. Fix and re-run
until green.

### 9. Publish

```bash
npm run chromatic   # reads CHROMATIC_PROJECT_TOKEN from the env
```

Report the Chromatic build URL for team-facing visual review/approval.
