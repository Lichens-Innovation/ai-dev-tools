...
| **hybrid (search)** | — | Combines **vector KNN** and **lexical BM25** hits, then fuses them in `fuseHybridChunkMatches` with fixed weights (**70%** vector, **30%** lexical) after **min–max** normalization per branch. |
| **min–max (normalization)** | min–max scaling | Rescales each query’s vector distances and BM25 scores into a comparable **[0, 1]** “similarity” range before weighted fusion (`hybrid-chunk-fusion.utils.ts`). |
