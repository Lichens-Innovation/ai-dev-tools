...
Exploratory vocabulary only (see TODO-010 in the README); not part of the current SQLite-backed implementation.

| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **RDF** | Resource Description Framework: a standard graph data model where facts are stated as **triples** (subject, predicate, object) and can be merged into an **RDF graph**. | linked data only (narrower) |
| **RDF graph** | The set of RDF triples viewed as nodes and labeled edges; the structured “source of truth” in an ontology-first retrieval design. | knowledge graph (informal; not always RDF) |
| **Triple** | One RDF statement: subject resource, predicate property, object (resource or literal). | tuple (different meaning in code) |
| **SPARQL** | The W3C query language for RDF: graph patterns, filters, and aggregations over a triple store (SPARQL 1.1 is the common version). | SPARQ, Sparql |
| **Triple store** | A database or service that stores RDF and answers SPARQL queries; may expose an HTTP SPARQL endpoint. | graph DB (many products are not RDF-native) |
| **Ontology** | A shared, formal specification of **classes** and **properties** (and often rules) that constrains how domain facts are represented in RDF. | glossary only (weaker than ontology) |
| **Ontology RAG** | A retrieval pattern where the **ontology** (RDF in a **triple store**) holds authoritative facts; an **LLM** translates natural-language questions into **SPARQL** (or refines queries) and explains bindings—not as the sole memory of the codebase. | ontology-only search (ignores complementary retrieval) |
