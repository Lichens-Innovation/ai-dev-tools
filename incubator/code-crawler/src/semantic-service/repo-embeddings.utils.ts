...
export type GetIndexedFileContentOutcome =
  | { error: string; httpStatus: number; ok: false }
  | { ok: true; payload: IndexedFileContentPayload };
...
export const getIndexedFileContentByFileId = async (fileId: string): Promise<GetIndexedFileContentOutcome> => {
  const metadata = workspaceSemanticIndexStore.getFileMetadataByFileId(fileId);
