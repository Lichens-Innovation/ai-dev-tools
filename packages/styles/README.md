...
- **Inter and Bodoni Moda are variable fonts**: one binary spans the whole weight axis, and the
  API answers every requested weight with the *same* file. The script groups faces by the file
...
3. Rebuild a consumer and confirm the new face is *painted*, not merely declared — see below.
...
which fonts the engine *painted with* — CDP's `CSS.getPlatformFontsForNode`, driven through the
