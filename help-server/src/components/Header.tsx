      ...
      <nav className="page-wrap grid grid-cols-[1fr_auto_1fr] items-center gap-4 py-3">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md p-1.5 text-(--ink-2) hover:bg-[var(--bg-3)] hover:text-(--ink)"
          >
            <span className="flex h-[14px] w-[18px] items-center justify-center">
              {sidebarOpen ? (
                <svg
                  width="14"
                  height="14"
          ...
          <Link
            to="/"
            className="shrink-0 text-[13px] font-semibold tracking-[-0.01em] text-(--ink) no-underline hover:text-[var(--amber)]"
          >
            AI Dev Tools
          </Link>
        </div>
        ...
        <div className="relative w-125 max-w-md">
          <div
            className={`flex items-center gap-2 rounded-md border bg-[var(--bg-2)] px-3 py-1.5 transition-colors ${
              focused
          ...
          >
            ...
            <input
              ...
              className="min-w-0 flex-1 bg-transparent text-[13px] text-(--ink) placeholder-[var(--ink-3)] outline-none"
            />
            {!focused && !inputValue && (
              <kbd className="hidden rounded-sm border border-[var(--line)] bg-[var(--bg-3)] px-1.5 py-0.5 text-[10px] text-[var(--ink-3)] font-sans sm:inline-block">
                {kbdHint}
              </kbd>
            )}
            {inputValue && (
              <button
                ...
                className="shrink-0 text-[var(--ink-3)] hover:text-(--ink)"
              >
                <svg
                  width="10"
                  height="10"
          ...
          {showResults && (
            <div className="absolute left-0 right-0 top-full z-[60] mt-1 overflow-hidden rounded-md border border-[var(--line)] bg-[var(--bg-2)] shadow-[var(--shadow-2)]">
              {results.length === 0 ? (
                <p className="px-4 py-3 text-[12px] text-[var(--ink-3)]">
                  No results found.
                </p>
              ) : (
                <ul
