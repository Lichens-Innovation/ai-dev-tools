// /docs — the GLOBAL documentation landing page: Maestro-app docs plus Claude Code concept docs.
//
// NOT gated on an open project — `window.maestro.data.globalDocs()` reads two fixed directories
// the app ships (`apps/maestro/docs/app/` and the repo-root `docs/`), never the open project's own
// `docs/`. That reader lives at `/project-docs` now (see routes/project-docs.index.tsx) — this
// route took over `/docs` per the hamburger menu's "Docs = the global page" decision.
...
function DocList({ group, docs }: { group: "app" | "claude-code"; docs: DocMeta[] }) {
        ...
        <div className="mx-auto max-w-3xl px-6 pb-16 pt-10 flex flex-col gap-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-[13px] text-subtle m-0">
                Maestro the app, and the Claude Code concepts it builds on — available with no project open. Search
                matches headings and body text, and opens the doc at the section that matched.
              </p>
            </div>
            <button
              type="button"
          ...
          {!result.ok ? (
            ...
            <>
              ...
              <section>
                <div className="mb-3 flex items-center gap-3">
                  <span className="section-label">Claude Code Concepts</span>
                  <span className="text-[10px] text-subtle">{result.value.claudeCode.length}</span>
                </div>
                <DocList group="claude-code" docs={result.value.claudeCode} />
              </section>

              <p className="flex items-center gap-1.5 text-[12px] text-subtle m-0">
                <BookOpen size={12} /> {result.value.app.length + result.value.claudeCode.length} documents ·{" "}
                {result.value.sections.length} sections indexed
              </p>
            </>
          )}
        </div>
