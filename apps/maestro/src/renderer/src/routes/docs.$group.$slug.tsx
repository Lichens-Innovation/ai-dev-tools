// /docs/$group/$slug — the GLOBAL documentation reader, over one of the two fixed corpora
// `window.maestro.data.globalDocs()`/`.globalDoc()` serve (Maestro-app docs, Claude Code concept
// docs). NOT gated on an open project.
//
...
export const Route = createFileRoute("/docs/$group/$slug")({
  ...
  loader: async ({ params }) => {
    const group = params.group === "claude-code" ? "claude-code" : "app";
    return {
      group: group as "app" | "claude-code",
  ...
  return (
    <SlidePanel isOpen={isOpen} onClose={onClose} side="left" widthClass="w-80" toggleDataAttr="data-docs-toggle">
      <div className="flex items-center border-b border-(--line) px-4 py-3">
        <span className="section-label">{group === "app" ? "Maestro" : "Claude Code Concepts"}</span>
      </div>
...
function GlobalDocPage() {
  ...
  const groupDocs = list.ok ? (group === "app" ? list.value.app : list.value.claudeCode) : [];
  ...
  return (
    <div className="w-full h-screen bg-(--bg) font-sans text-(--ink) flex flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-(--line) shrink-0">
        ...
        >
          <PanelLeft size={13} /> {group === "app" ? "Maestro docs" : "Claude Code docs"}
        </button>
        <Link
          to="/docs"
