...
| Where's the install logic?                             | `installRuntime()`/`uninstallRuntime()` in `src/core` (the app's `/maestro` route), and `plugins/ai-tools-manager/scripts/maestro-install.js` for the terminal path                                                                                                                                            |
...
- **Two things can register Maestro's hooks, and both firing is a visible bug.** A project installed from the app's `/maestro` route has them in its own `.claude/settings.json`; the `ai-tools-manager` plugin registers the same ones globally from `hooks.json`. With both, every tool call is logged twice and every subagent gets its context injected twice. `InstallStatus.pluginHooksActive` detects it and the `/maestro` route says so — it does not "fix" it, because the fix is in the user's global configuration.
