...
- `plugins/ai-tools-manager/scripts/maestro-install.js` — **scaffold-only**: installs the `maestro` skill (copied whole when absent; otherwise its `Maestro:STEPS` / `Maestro:PRINCIPLES` managed regions are re-synced from `templates/maestro/SKILL.md` — see `scripts/lib/maestro-skill-regions.cjs`), copies the runtime scripts, merges the `bash-validation.sh` PreToolUse Bash hook into `.claude/settings.json`, copies `bash-validation.sh` into the project, and gitignores the ephemeral session files at the repo root via an `# Maestro` section with `**/.claude/maestro_session*.{json,jsonl}` globs (the `**/` prefix covers root-level `.claude/` too, so no per-package `.claude/.gitignore` is needed). Does **not** render — that needs `maestro.json` and is done afterward by `maestro-render-orchestrator.cjs`
...
- `packages/maestro-core/` — framework-free TypeScript home of all node-side Maestro logic (config read/merge/write, success-path derivation, orchestrator render, managed-region sync, session-log helpers). **`scripts/lib/maestro-session.cjs` and `scripts/lib/maestro-skill-regions.cjs` are GENERATED from it** by `pnpm --filter @repo/maestro-core build:plugin-libs` — edit the TS source, never those two `.cjs` files, or your change is overwritten on the next build. The bundles exist because the hook scripts run under bare `node` in a user's project with no `node_modules`. Parity + byte-identity tests live in `packages/maestro-core/test/`.
