---
name: log-view
description: "Explains how the /session-log view in ai-tools-manager is built end-to-end: the left instance card-diagram, the right humanized log pane, the hover input/output popup, how log entries map to Instance segments, and how the afk-session-log.js / afk-subagent-log.js hooks write the afk_session.log.jsonl it reads. Use when the user is working inside apps/ai-tools-manager and asks how the session-log view works, how cards/instances are derived, where SUCCESS/FAILURE comes from, why the log is empty, why a card has no status badge, or how dispatch/handoff entries are produced by the hooks."
---

# Log View

The `/session-log` route (`src/routes/session-log.tsx`) is a **read-only debugger** for AFK workflow sessions. It reads `<cwd>/.claude/afk_session.log.jsonl` and presents it in two panes: a **left vertical card diagram** (one card per instance occurrence — main session + each subagent run, with a SUCCESS/FAILURE badge) and a **right humanized log** (tool calls as readable English, grouped by instance). Clicking a card scrolls the log to where that instance started. Hovering a subagent card opens a popup showing the full input message (what the main session told the agent) and output message (the agent's full final message including its `HANDOFF:` line) — the primary debugging view.

This is the **read side** of the AFK runtime: it displays what the hook scripts write. See the `afk-architecture` skill for the **write side** (how the log is produced and the full HANDOFF routing contract). See the `workflow-view` skill for the sibling `/workflows` authoring view.
  ...
  <SessionLogProvider>  (src/utils/session-log-context.tsx)
    └── EventSource("/api/session-log-stream")  ← one persistent SSE connection
          server: src/routes/api/session-log-stream.ts
            polls afk_session.log.jsonl every 1 s (server-side)
...
TopNav (every AFK page): "● Session Log" dot driven by useSessionLog().connected
```
...
```
Active AFK session
  PreToolUse hook   → afk-session-log.js  → tool-call entry (ts, origin, log)
  SubagentStart     → afk-subagent-log.js → dispatch entry  (kind:"dispatch", agent, agent_id, input)
  SubagentStop      → afk-subagent-log.js → handoff entry   (kind:"handoff", agent_id, status, label, output)
  SessionEnd        → afk-session-cleanup.sh → DELETE afk_session.log.jsonl
```

All three hooks append to the same file. All are no-ops when `afk.json` is absent. The file is deleted at SessionEnd, so the empty state is normal.
...
| Log reader server fn + `resolveLogFile` + `parseLogLines` + `SessionLogEntry` | `src/utils/afk-session-log.ts` |
...
| Docker-aware cwd helpers (`readCwd`, `mountedProjectPath`) | `src/utils/afk-fs.ts` |
| `titleFromName` — origin string → display name | `src/utils/text.ts` |
| **Writer — tool-call entries** (PreToolUse, matcher `.*`) | `plugins/ai-tools-manager/scripts/afk-session-log.js` |
| **Writer — dispatch + handoff entries** (SubagentStart/Stop, matcher `.*`) | `plugins/ai-tools-manager/scripts/afk-subagent-log.js` |
| Shared append helper (`appendSessionLog`, `readStdin`) | `plugins/ai-tools-manager/scripts/lib/afk-session.cjs` |
| Hook registration (all three hooks registered here) | `plugins/ai-tools-manager/hooks/hooks.json` |
| Source file (ephemeral, append-only, gitignored) | `<cwd>/.claude/afk_session.log.jsonl` |
...
A missing `kind` = a plain tool-call entry from `afk-session-log.js`. The schema is **backward compatible**: old logs (tool-call-only) parse and display correctly; `dispatch`/`handoff` fields simply don't appear.
...
- **Hover popup** — only rendered for subagent cards where `inst.input || inst.output` is non-null (needs dispatch/handoff entries from `afk-subagent-log.js`). Implemented via `hoveredId` state + `onMouseEnter`/`onMouseLeave` on both the card wrapper and the popup (so moving from card to popup doesn't dismiss it). Popup has two scrollable `<pre>` sections — **Input** and **Output** — styled `max-h-[80vh] overflow-y-auto w-[30rem] bg-(--bg) border border-(--line) rounded-lg shadow-xl`. Shows "no message captured" when a side is null.
...
- The main session itself segments into multiple "Main Session" cards when subagents interleave (normal for sequential AFK dispatch).
- Parallel subagents would fragment, but AFK runs agents sequentially, so interleaving is rare.
...
### `afk-session-log.js` — tool-call entries (PreToolUse)
...
- **Gate:** exits immediately if `<cwd>/.claude/afk.json` doesn't exist (AFK not configured).
...
### `afk-subagent-log.js` — dispatch + handoff entries (SubagentStart/Stop)
...
- Same gate and append-only pattern as `afk-session-log.js`.
...
"PreToolUse":  [{ "matcher": ".*", "hooks": [
  { "command": "node", "args": ["${CLAUDE_PLUGIN_ROOT}/scripts/afk-session-log.js"] }
]}]
```
...
`afk-session-cleanup.sh` (SessionEnd hook) deletes both `afk_session.log.jsonl` and `afk_session.json`. The page's empty state is therefore the expected condition when no AFK session is active — it is not an error.

### Why SUCCESS/FAILURE requires `afk-subagent-log.js`

The plain tool-call log from `afk-session-log.js` has **no outcome data** — it records only that a tool was called. The subagent's `HANDOFF:` line lives in its transcript (the `last_assistant_message` at SubagentStop time), which is only accessible to a SubagentStop hook. Without `afk-subagent-log.js`, all instance `status` fields would be `null` and no badges would render. See `afk-architecture` for the full HANDOFF routing contract and how the orchestrator uses the same label to route the workflow.
...
- **`reset` event on SessionEnd.** When the JSONL file disappears (deleted by `afk-session-cleanup.sh`), the server emits `reset: {}` and `lineCount` drops to 0. The provider clears `entries`, the page shows the empty state. A new session's `init` event re-fills it. This is the normal SessionEnd → new session cycle without a page reload.
...
- **`getAfkSessionLog` server fn is kept for SSR.** On a hard reload the provider starts with `entries: []` until the first `init` event arrives. The route no longer uses a `loader`, so there is a brief empty state flash on page load — acceptable for a debugging tool.
...
- **`afk-subagent-log.js` runs from the plugin dir, not the project copy.** Unlike `afk-set-session-workflow.cjs` and `afk-render-orchestrator.cjs` (which are copied into `.claude/scripts/` at install time), the SubagentStart/Stop scripts run directly from `${CLAUDE_PLUGIN_ROOT}/scripts/`. Editing `afk-subagent-log.js` takes effect immediately for all projects. Adding or removing the hook registration in `hooks.json` requires a new Claude session to pick up.
